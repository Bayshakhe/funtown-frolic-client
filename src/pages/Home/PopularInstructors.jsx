      ...
      {instructors.map((i,index) => (
        ...
        <figure>
          ...
          <div className="card-body">
            ...
            <h2 className="card-title text-2xl text-teal">{i.instructorName}</h2>
            <p className="opacity-60 text-right">{i.instructorEmail}</p>
            </div>
            <p className="font-semibold">{i.className}</p>
            <p>Experienced as a professional {i.className}r for {index+2} years.</p>
          </div>
        </div>
      ))}
    </div>
